...
use Unirgy\Dropship\Model\Label\Ups as LabelUps;

class Ups extends LabelUps
{
    /**
     * @var \Unirgy\Rma\Model\Rma
     */
    protected $_rma;
    public function requestRma($track)
    {
        ...
        $request->setNode('Shipment/Shipper/Address/AddressLine1', substr(trim($a->getStreetLine(1)), 0, 35));
        $request->setNode('Shipment/Shipper/Address/AddressLine2', substr(trim($a->getStreetLine(2)), 0, 35));
        $request->setNode('Shipment/Shipper/Address/AddressLine3', substr(trim($a->getStreetLine(3)), 0, 35));
        ...
        if ($this->_configScopeConfigInterface->isSetFlag('carriers/ups/negotiated_rates', ScopeInterface::SCOPE_STORE, $store)) {
        ...
        if ($fromCountry!=$toCountry) {
            ...
            $number = 0;
            foreach ($this->_rma->getAllItems() as $item) {
                $oItem = $this->_rma->getOrder()->getItemById($item->getOrderItemId());
...
EOT;
        ...
        $response = new Element($xmlResponse);
#$this->_logLoggerInterface->debug($response);
        $this->_validateResponse($response);
