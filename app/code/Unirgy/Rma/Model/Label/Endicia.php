...
use Unirgy\Dropship\Model\Label\Endicia as LabelEndicia;

class Endicia extends LabelEndicia
{
    public function requestRma($track)
    {
        ...
        $endiciaData = new DataObject($endiciaData);
        ...
        //$usps = $this->_modelConfig->getCarrierInstance('usps');
        ...
        $data = [
            ...
            'ReturnAddress1' => $address->getStreetLine(1),
            'ReturnAddress2' => $address->getStreetLine(2),
            'ReturnAddress3' => $address->getStreetLine(3),
            'ReturnAddress4' => $address->getStreetLine(4),
...
*/
        ...
        $result = $client->GetPostageLabel(['LabelRequest'=>$data]);
        ...
        if (empty($xml->Base64LabelImage) && empty($xml->Label->Image)) {
            throw new \Exception('Unable to retrieve the label.');
        }

        $track->setCarrierCode('usps');
        $track->setTitle('USPS');
        $track->setTrackNumber($xml->TrackingNumber);
        ...
        if ($v->getLabelType()=='PDF' && $labelType=='International') {
            ...
            $track->setLabelRenderOptions(serialize([
                'r' => 90,
                'l' => .5,
                't' => .5,
                'w' => 10,
        ...
        if ($balanceThreshold && $recreditAmount && $xml->PostageBalance<=$balanceThreshold) {
            try {
                ...
                $this->_logLoggerInterface->debug('Unable to recredit Endicia account: '.$e->getMessage());
            }
        }
