...
class Configurable extends DefaultPrice
{
    ...
    protected function _applyConfigurableOption()
    {
        ...
        foreach ($canStates as $csKey=>$csLbl) {
            $select->columns([
                'udmp_'.$csKey.'_min_price' => new \Zend_Db_Expr('i.udmp_'.$csKey.'_min_price'),
                'udmp_'.$csKey.'_max_price' => new \Zend_Db_Expr('i.udmp_'.$csKey.'_max_price'),
                'udmp_'.$csKey.'_cnt' => new \Zend_Db_Expr('i.udmp_'.$csKey.'_cnt'),
            ]);
        }
        ...
        foreach ($canStates as $csKey=>$csLbl) {
            $select->columns([
                new \Zend_Db_Expr('MIN(udmp_'.$csKey.'_min_price)'),
                new \Zend_Db_Expr('MAX(udmp_'.$csKey.'_max_price)'),
                new \Zend_Db_Expr('SUM(udmp_'.$csKey.'_cnt)'),
            ]);
        }
        ...
        foreach ($canStates as $csKey=>$csLbl) {
            $select->columns([
                'udmp_'.$csKey.'_min_price' => new \Zend_Db_Expr('io.udmp_'.$csKey.'_min_price'),
                'udmp_'.$csKey.'_max_price' => new \Zend_Db_Expr('io.udmp_'.$csKey.'_max_price'),
                'udmp_'.$csKey.'_cnt' => new \Zend_Db_Expr('io.udmp_'.$csKey.'_cnt'),
            ]);
        }
