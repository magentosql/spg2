...
class Grouped extends DefaultPrice
{
    /**
    ...
    protected function _prepareGroupedProductPriceData($entityIds = null)
    {
        ...
        foreach ($canStates as $csKey=>$csLbl) {
            $minCheckSql = $connection->getCheckSql('le.required_options = 0', 'i.udmp_'.$csKey.'_min_price', 0);
            $maxCheckSql = $connection->getCheckSql('le.required_options = 0', 'i.udmp_'.$csKey.'_max_price', 0);
            $cntCheckSql = $connection->getCheckSql('le.required_options = 0', 'i.udmp_'.$csKey.'_cnt', 0);
            $select->columns([
