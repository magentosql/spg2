 ...
 */
class Bundle extends DefaultPrice
{
    /**
    ...
    protected function _prepareBundlePriceByType($priceType, $entityIds = null)
    {
        ...
        if ($this->moduleManager->isEnabled('Magento_Tax')) {
            $taxClassId = $this->_addAttributeToSelect($select, 'tax_class_id', 'e.'.$rowIdField, 'cs.store_id');
        } else {
        ...
        $this->_addAttributeToSelect($select, 'price_type', 'e.'.$rowIdField, 'cs.store_id', $priceTypeCond);

        $price = $this->_addAttributeToSelect($select, 'price', 'e.'.$rowIdField, 'cs.store_id');
        $specialPrice = $this->_addAttributeToSelect($select, 'special_price', 'e.'.$rowIdField, 'cs.store_id');
        $specialFrom = $this->_addAttributeToSelect($select, 'special_from_date', 'e.'.$rowIdField, 'cs.store_id');
        $specialTo = $this->_addAttributeToSelect($select, 'special_to_date', 'e.'.$rowIdField, 'cs.store_id');
    ...
    protected function _calculateBundleSelectionPrice($priceType)
    {
        ...
        $select = $connection->select()->from(
            ...
            'bo.parent_id = parent_product.'.$rowIdField,
            ['option_id']
        )->join(
    ...
    protected function _prepareTierPriceIndex($entityIds = null)
    {
        ...
        $select = $connection->select()->from(
            ...
            ['tp.'.$rowIdField, 'cg.customer_group_id', 'cw.website_id']
        );

        if (!empty($entityIds)) {
            $select->where('tp.'.$rowIdField.' IN(?)', $entityIds);
        }
