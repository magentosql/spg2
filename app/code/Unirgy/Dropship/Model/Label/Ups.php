    ...
    implements CarrierInterface
{
    ...
    public function requestLabel($track)
    {
        ...
        $upsData = new DataObject($upsData);
        ...
        $weightUnit = $this->_hlp->getScopeConfig('carriers/ups/unit_of_measure', $store);
        $shipperNumber = substr($v->getUpsShipperNumber() ? $v->getUpsShipperNumber() : $this->_hlp->getScopeConfig('carriers/ups/shipper_number', $store), 0, 6);

        $request = new Element('<ShipmentConfirmRequest/>');
        ...
        if ($this->_hlp->getScopeConfig('carriers/ups/dest_type', $store)=='RES') {
            $request->setNode('Shipment/ShipTo/Address/ResidentialAddress', '');
        }
        if ($this->_hlp->getScopeFlag('carriers/ups/negotiated_rates', $store)) {
        ...
        if ($upsData->getUpsVerbalConfirmation()) {
            $request->setNode('Shipment/Package/PackageServiceOptions/VerbalConfirmation/ContactInfo/Name', substr($this->_hlp->getScopeConfig('carriers/ups/shipper_attention', $store), 0, 35));
            $request->setNode('Shipment/Package/PackageServiceOptions/VerbalConfirmation/ContactInfo/PhoneNumber', substr($this->_hlp->getScopeConfig('carriers/ups/shipper_phone', $store), 0, 15));
        }
        ...
        default:
            throw new \Exception('Invalid vendor label type');
        }
        ...
        $response = new Element($xmlResponse);
        $this->_validateResponse($response);

        $shipmentDigest = (string)$response->descend('ShipmentDigest');
        if (!$shipmentDigest) {
            throw new \Exception("Could not retrieve shipment digest vaue.");
        }
...
EOT;
        ...
        $response = new Element($xmlResponse);
#$this->_logLoggerInterface->log($response);
        $this->_validateResponse($response);

        $xmlPackages = $response->descend("ShipmentResults/PackageResults");
        if (!$xmlPackages) {
            throw new \Exception('Could not retrieve shipping labels.');
        }
        ...
        $track->setTitle($this->_hlp->getScopeConfig('carriers/ups/title', $store));
        $track->setTrackNumber($tracking);
    ...
    {
        if (!$this->_hlp->getScopeConfig('udropship/vendor/void_labels')) {
            return $this;
        }
        ...
        $request = new Element('<VoidShipmentRequest/>');
        $request->setNode('Request/TransactionReference/CustomerContext', $orderId);
    ...
    {
        if (!$response) {
            throw new \Exception(__('Error parsing confirmation response'));
        }

        if ((int)$response->descend('Response/ResponseStatusCode') !== 1)  {
            throw new \Exception((string)$response->descend('Response/Error/ErrorCode').': '.(string)$response->descend('Response/Error/ErrorDescription'));
        }

        if ($validateContext) {
            $tref = (string)$response->descend('Response/TransactionReference/CustomerContext');
            if ($tref != $this->_order->getIncrementId())  {
                throw new \Exception("Transaction reference '".$tref."' received in response does not match transaction reference '".$this->_order->getIncrementId()."' that was sent in request.");
            }
        }
    }
    ...
    protected function _callShippingXml($call, $request)
    {
        ...
        if (!$baseUrl) {
            $baseUrl = $this->_hlp->getScopeConfig('carriers/ups/gateway_xml_url');
        }
        $baseUrl = preg_replace('#^(.*/ups\.app/xml).*$#', '$1', $baseUrl);

        $response = $this->_hlp->curlCall($baseUrl.'/'.$call, $request);
    ...
    public function processImage($type, $labelImage, $data=null)
    {
        ...
        case 'PDF':
            ...
            if(!$im) {
                throw new \Exception('Missing or incorrect image file: '.$gifFile);
            }
            ...
            if (!imagepng($im, $pngFile)) {
                throw new \Exception('Error while saving to PNG');
            }
            imagedestroy($im);
...
ITEMS: %items$s';
    ...
    public function collectTracking($v, $trackIds)
    {
        ...
        $request = new Element('<TrackRequest/>');
    ...
    public function getDeliveryDateAll($vendor, $address, $weight, $value, $shipDate)
    {
        ...
        $request = new Element('<TimeInTransitRequest/>');
        $request->setNode('Request/TransactionReference/CustomerContext', '1');
        $request->setNode('Request/TransactionReference/XpciVersion', '1.0');
